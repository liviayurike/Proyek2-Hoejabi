...
$cek=mysqli_query($conect, "select*from tb_keranjang where id_keranjang='$id_keranjang' and id_produk='$id_p'")or die(mysqli_error($conect));
...
if($ada==0){
    mysqli_query($conect, "insert into tb_keranjang (id_keranjang,id_produk,qty,subtotal,sub_berat)values('$id_keranjang','$id_p','$qty','$tom','$berate')")or die(mysqli_error($conect));
    echo"<script>;document.location='keranjang.php'</script>";
}
else
{    $sql=mysqli_fetch_array(mysqli_query($conect, "select* from tb_produk where id_produk='$id_p'"));
    if($sql['stok_produk']> $query['qty']) {
    mysqli_query($conect, "update tb_keranjang set qty=qty+1, subtotal=subtotal+$disk_t, sub_berat=sub_berat+$produk[berat_produk] where id_troli='$id_keranjang' and id_produk='$id_p'")or die(mysqli_error($conect));
    echo "
